|feature name|example|include?|AST?|
|-
...
|`then` introduces block|if a then b|✓|
|@-vars|@a|✓|
...
|external constructors|class A then constructor: c|✗|
|try-catch-finally| |✗|
|try-finally| |✗|
|try|try a|✗|
