...
|Thu, 7 June||choose a platform for building the bootstrapping compiler; haskell? jashkenas/coffee-script?|
|Fri, 8 June||implement the AST constructors|
|Mon, 11 June||choose a parsing strategy and library|
|Fri, 15 June||skip the CS-to-JS conversion for now and work on a simple code generator for CS ASTs, cscodegen, named after the ECMAScript code generator that we will be using, escodegen|
|Wed, 27 June||define a parser that can parse my subset of CoffeeScript and generate CoffeeScript code|
