...
|Fri, 10 August|Fri, 10 August|start writing the transformations from CS ASTs to JS ASTs|
||in progress|start pulling in the test suite from jashkenas/coffee-script|
||in progress|use Mozilla's source map generator to generate source maps|
|||finish up cscodegen|
